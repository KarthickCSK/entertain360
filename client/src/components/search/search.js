...
class Search extends Component {
  ...
  searchMovie = () => {
    ...
    axios({
      method: "get",
      url: `${REACT_APP_SERVER + REACT_APP_MOVIE_SEARCH}?keyword=${typedText ||
        "avenger"}&page=${currentPage}&adult=${false}`
    })
      .then(response => {
        // console.log(response)
        this.setState({
          loadingSearch: false,
          searchMovie: response.data.data.results,
  ...
  render() {
    ...
    return [
      <div
        className="form-row justify-content-center search-box"
        id="search-box"
      >
        <div className="col-6 col-md-4 col-sm-12 input-group">
          <div className="input-group-prepend">
            <div className="input-group-text">
              <input
                ...
                id="adult"
                value={adult}
                onChange={e => this.setState({ adult: !this.state.adult })}
              />
            </div>
          </div>
          <input
          ...
          <div className="input-group-append">
            <button
              className="btn cust-btn btn-secondary"
              type="button"
              id="search-btn"
              ...
              return (
                <div className="col-3" key={index}>
                  <IndiHorizontalCard
                    movie={movie}
                    fav={this.props.fav}
                    wantToWatch={this.props.wantToWatch}
                    toggleFav={this.props.toggleFav}
                    toggleWantToWatch={this.props.toggleWantToWatch}
                  />
                </div>
              );
            })}
